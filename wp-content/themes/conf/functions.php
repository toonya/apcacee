...
function remove_menus() {
    global$menu;
    $restricted = array(__('Dashboard'),__('Posts'), __('Media'), __('Links'),_('Comments'),_('Users'),_('Tools'),__('Appearance'),__('Plugins'),__('Settings'));
    end ($menu);
    while (prev($menu)){
        $value = explode(' ',$menu[key($menu)][0]);
        if(in_array($value[0] != NULL?$value[0]:"" , $restricted)){unset($menu[key($menu)]);}
    }
    // add posts option & rename 'News'
    add_menu_page('News', 'News', 'manage_options', 'edit.php', '','',1);
}
...
<?php include_once('conf_detail.php'); ?>
<?php include_once('section-page/section-page.php'); ?>
